...
// HttpClient cho ChatGPT API
builder.Services.AddHttpClient<ChemistryAPI.Services.ChatGptService>();

// ChatGPT Service
builder.Services.AddScoped<ChemistryAPI.Services.ChatGptService>();

// In-Memory Cache + Chat Rate Limit
builder.Services.AddMemoryCache();
builder.Services.AddSingleton<ChemistryAPI.Services.ChatRateLimitService>();
...
using (var scope = app.Services.CreateScope())
{
    ...
    // Seed admin user and courses
    var dbContext = scope.ServiceProvider.GetRequiredService<ChemistryDbContext>();
    await ChemistryAPI.SeedData.SeedAdminUserAsync(dbContext);
    await ChemistryAPI.SeedData.SeedCoursesAsync(dbContext);
}
